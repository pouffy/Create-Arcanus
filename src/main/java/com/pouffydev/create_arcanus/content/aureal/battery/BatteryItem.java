...
public class BatteryItem extends BlockItem {
    ...
    private void tryMultiPlace(BlockPlaceContext ctx) {
        ...
        boolean creative = getBlock().equals(CABlocks.creativeAurealBattery.get());
        BatteryBlockEntity batteryAt = ConnectivityHandler.partAt(
                creative ? CABlockEntities.creativeBattery.get() : CABlockEntities.battery.get(), world, placedOnPos
        );
        if (batteryAt == null)
            return;
        ...
        for (int xOffset = 0; xOffset < width; xOffset++) {
            for (int zOffset = 0; zOffset < width; zOffset++) {
                ...
                if (BatteryBlock.isBattery(blockState))
                    continue;
                if (!blockState.getBlock().canBeReplaced(blockState, BlockPlaceContext.at(ctx, offsetPos, face)))
                    return;
                batteriesToPlace++;
            }
        }
